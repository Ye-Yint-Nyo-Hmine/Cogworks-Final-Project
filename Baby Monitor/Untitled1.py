...
outfile = "db.npy"
db = np.load(outfile,allow_pickle=True)
db
...
db.size, db.shape
num_fp_per_aud = [len(i) for i in db] #num of fingerprints in each audio
unique_fp_per_aud = set(num_fp_per_aud)
len(unique_fp_per_aud) # 45 unique values
#cant turn db in np array


# In[15]:


directory_path = "data/crying/"
N = [i for i, file in enumerate(os.listdir(directory_path))][-1]
N
...
listen_time = 7.5
frames, samplerate = record(listen_time)
